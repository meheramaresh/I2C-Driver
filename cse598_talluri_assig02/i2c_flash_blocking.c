...
/* Function Protypes */
int eeprom_write(struct i2c_client *client, char *buf, int pageCount, short currentAddr);
int eeprom_read(struct i2c_client *client, char *buf, int pageCount, short currentAddr);
...
static struct workqueue_struct *i2cflash_wq;	/* Work queue for i2c_flash */
...
int eeprom_write(struct i2c_client *client, char *buf, int pageCount, short currentAddr){
	...
	/* Define memory for the new buffer */
	tmpBuf = kzalloc(64 + 2, GFP_KERNEL);

	if(pageCount > 0){
		for(loopCount = 0; loopCount < pageCount; loopCount++){
		
			/* Assign address at the first two bytes */
			printk("\nWriting to address 0x%x\n", currentAddr);
			*tmpBuf = (currentAddr >> 8) & 0x00FF; 		/* Save the higher byte */
		...
		*(tmpBuf + 1) = currentAddr & 0x00FF;		/* Save the lower byte */
		...
		tempRet = i2c_master_send(client, tmpBuf, 2);
		if(tempRet < 0){
			printk("Setting address failed\n");
			ret = FAILURE;		
		}
	}
	else{
		printk("Invalid page count\n");
		ret = FAILURE;
	}
...
ssize_t i2cflash_driver_write(struct file *file, const char *buf,
	...
	/* Write to the EEPROM */	
	ret = eeprom_write(client, tmp, count, g_currentAddr);
	if(ret > 0){
		/* Update the current write address */
		g_currentAddr += (count * 64);
		g_currentAddr %= 0x8000;
		printk("\nNew current write from int %x\n", g_currentAddr);
	}
...
int eeprom_read(struct i2c_client *client, char *buf, int pageCount, short currentAddr){
	...
	ret = eeprom_write(client, buf, 0, currentAddr);

	/* Read the data from EEPROM */
        ret = i2c_master_recv(client, buf, pageCount * 64);
	if(ret > 0)
		ret = SUCCESS;
...
ssize_t i2cflash_driver_read(struct file *file, char *buf,
	...
	/* Allocate memory */
        tmp = kzalloc(byteCount, GFP_KERNEL);
        if (tmp == NULL)
                return -ENOMEM;

	eeprom_read(client, tmp, count, g_currentAddr);
		...
		/* Copy the data to the user buffer */
                ret = copy_to_user(buf, tmp, byteCount) ? -EFAULT : ret;
	}
...
int __init i2cflash_driver_init(void){
	...
	/* Create work queue */
	if(i2cflash_wq == NULL){
		i2cflash_wq = create_workqueue("i2c_flash_work_queue");	
	}
...
void __exit i2cflash_driver_exit(void){
	...
	if(i2cflash_wq != NULL){
		...
		flush_workqueue(i2cflash_wq);
	
		/* Destroy the work queue */
		destroy_workqueue(i2cflash_wq);
	}
